  ...
  const bubbleContent = (
    <View style={{ alignItems: isUser ? 'flex-end' : 'flex-start' }}>
      {replyToMessage && !isUser && (
        <View style={{ maxWidth: '100%', paddingHorizontal: Spacing.md }}>
  ...
  if (onLongPress) {
    return (
      <View style={[styles.row, { justifyContent: isUser ? 'flex-end' : 'flex-start' }]}>
        <TouchableOpacity activeOpacity={0.8} onLongPress={() => onLongPress(message)} delayLongPress={400} style={[styles.bubbleWrapper, { maxWidth: isUser ? '85%' : '96%', alignItems: isUser ? 'flex-end' : 'flex-start' }]}>
          {bubbleContent}
        </TouchableOpacity>
      </View>
    );
